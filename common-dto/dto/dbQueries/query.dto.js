...
exports.queries = {
    ...
    getRoles: "select r.* from roles r join user_role ur on ur.role_id = r.roles_id where ur.user_id = $1",
    getRolesPermission: 'SELECT "roleId", "permissionId", p."name" from role_permissions rp join permissions p on p."id" = rp."permissionId" where rp."roleId" in(select r.roles_id from roles r join user_role ur on ur.role_id = r.roles_id where ur.user_id = $1)',
    ...
    getPastAppointmentsWithPagination: `SELECT * FROM appointment WHERE (("appointment_date" <= $2 AND "status"=$5) OR("appointment_date" <= $7 AND "startTime" <= $8 AND ("status"=$6 OR "status"= $9))) AND "patient_id" = $1 AND "is_cancel"=false order by appointment_date desc , appointment."startTime" asc limit $4 offset $3`,
    ...
    getPatientDetails: 'SELECT "patient_id", "is_token_expired", "firstName","lastName","email","dateOfBirth","email", "phone" from patient_details where patient_id = $1',
    getPatientDetailsByEmail: "SELECT count(*)  from patient_details where email = $1",
    changePatientTokenExpiry: "SELECT is_token_expired from patient_details where patient_id =$1",
    changeDoctorTokenExpiry: 'SELECT is_token_expired from doctor where "doctorId" = $1',
    setDoctorTokenExpiry: 'update doctor set "is_token_expired"= true where "doctorId" =$1',
    setPatientTokenExpiry: 'update patient_details set "is_token_expired" = true where patient_id =$1',
    patientTokenExpiryFinal: 'update patient_details set "is_token_expired" = false where patient_id =$1 ',
    doctorTokenExpiryFinal: 'update doctor set "is_token_expired" = false where "doctorId" =$1',
    ...
    getPastAppointments: `SELECT * FROM appointment WHERE (("appointment_date" <= $2 AND "status"= $3) OR("appointment_date" <= $5 AND "startTime" <= $6 AND ("status"=$4 OR "status"= $7))) AND "patient_id" = $1 AND "is_cancel"=false `,
    ...
    getUpcomingAppointmentsCounts: `SELECT * FROM appointment a WHERE ((a."appointment_date" = $5 and a."startTime" >= $6) or (a."appointment_date" >= $2)) AND (a."patient_id" = $1 or (a.attender_mobile = (select phone from patient_details where patient_id = $1))) AND (a."status"= $3 OR a."status"= $4) AND a."is_cancel"=false order by appointment_date asc , a."startTime" asc `,
